...
plackett = { ensembletype: {"A": [], "B": [], "C": [], "D": []} for ensembletype in ensembletypes }
bradley = { ensembletype: {"A": [], "B": [], "C": [], "D": []} for ensembletype in ensembletypes }
crossover = { ensembletype: {"A": [], "B": [], "C": [], "D": []} for ensembletype in ensembletypes }
cambridge = { ensembletype: {"A": [], "B": [], "C": [], "D": []} for ensembletype in ensembletypes }

modelnames = list(zip(
    [plackett, bradley, crossover, cambridge],
    ["plackett-luce", "bradley-terry", "crossover", "cambridge"]
))

for ensembletype, ensemble in zip(ensembletypes, ensembles):
    for plan in ensemble:
        for scenario, concentration in scenariomap.items():
            for model, name in modelnames:
                ...
                statewide = {
                    id: [
                        c for c in configurations if c.model == name and c.concentration == concentration
                    ]
                ...
                for pool in range(4):
                    ...
                    for indexset in indices:
                        ...
                        model[ensembletype][scenario].append(sum(results.flat))

# Now we want to figure out the *frequencies* of each seat total.
for model, name in modelnames:
    for ensemble in ensembletypes:
        for scenario in scenariomap.keys():
            total = len(model[ensemble][scenario])
            model[ensemble][scenario] = {
                seats: count/len(model[ensemble][scenario])
                for seats, count in dict(Counter(model[ensemble][scenario])).items()
            }
...
scenarioplots = list(zip([tl, tr, bl, br], ["A", "B", "C", "D"]))
...
for ax, scenario in scenarioplots:
    x = 1
    ...
    for model, name in modelnames:
        for ensemble in model.values():
            for y, share in ensemble[scenario].items():
...
for ax, scenario in scenarioplots:
    ...
    ax.set_yticks(
        yticks,
        [str(int(t)) if t >=0 else "" for t in yticks]
    )
    ...
    # Set title.
    ax.set_title(scenario, fontdict=lfd)
...
fig.tight_layout()
# plt.show()
plt.savefig(output/"plots-by-scenario-by-model.png", dpi=600, bbox_inches="tight")
