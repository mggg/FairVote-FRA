...
for ordering, states in stateorders:
    ...
    for state in states:
            ...
            for ensembletype in ensembletypes:
                ...
                if tpath.exists():
                    for plan in range(50 if representatives > 5 else 1):
                        try:
                            districts = []
                            with jsonlines.open(planpath/f"neutral-{plan}.jsonl") as r:
                                for district in r:
        ...
        r = aggregate(ensembles[0], models, concentrationnames, subsample=subsample)["all"]
    ...
    fig.tight_layout()
    # plt.show()
    plt.savefig(f"./output/figures/nationwide-{ordering}.png", dpi=600, bbox_inches="tight")
    plt.clf()
